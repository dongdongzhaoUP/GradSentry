...
from typing import *
from transformers import AutoConfig, AutoTokenizer, AutoModelForCausalLM, LlamaForCausalLM, MptForCausalLM, GPT2LMHeadModel, GenerationConfig, Qwen2ForCausalLM, MistralForCausalLM, GemmaForCausalLM, GlmForCausalLM, GPTJForCausalLM, Phi3ForCausalLM, GPTNeoXForCausalLM, FalconForCausalLM
...
class CasualLLMVictim(Victim):
    ...
    def __init__(
        ...
        update_lm_head:Optional[bool] = False,
        **kwargs,
    ):
        ...
        # Use bfloat16 + flash_attention_2 for memory efficiency and speed
        if self.model_name == "mistral" or self.model_name == "gemma" or self.model_name == "qwen" or self.model_name == "falcon":
            self.llm=AutoModelForCausalLM.from_pretrained(path, config=self.model_config, trust_remote_code=True, device_map="auto",torch_dtype=torch.bfloat16)
        else:
        ...
        if isinstance(self.llm, LlamaForCausalLM):
            ...
            if self.tokenizer.pad_token is None:
                self.tokenizer.pad_token = self.tokenizer.eos_token
                self.tokenizer.pad_token_id = self.tokenizer.eos_token_id
        elif isinstance(self.llm, GPTNeoXForCausalLM):
            self.tokenizer.pad_token = self.tokenizer.eos_token
            self.tokenizer.padding_side = 'left'
            self.tokenizer.pad_token_id = self.tokenizer.eos_token_id
    ...
    def resetPETPara(self):
        if self.peftConfig.get('lora') and self.peftConfig.get('loraConfig') is not None:
            ...
            for n,p in self.llm.named_parameters():
                if self.targetPara is not None and self.targetPara in n:
        ...
        if (
            ...
            if self.model_name == "glm":
                loraConfig = LoraConfig(
                    **self.peftConfig.get("loraConfig"), task_type=TaskType.CAUSAL_LM,target_modules=["q_proj", "k_proj", "v_proj"]
                )
            else:
                loraConfig = LoraConfig(
            ...
            # Optionally enable lm_head training
            if self.update_lm_head:
                for n, p in self.llm.named_parameters():
                    if "lm_head" in n:
                        p.requires_grad_(True)
                        print(f"Enabled training for: {n}")
