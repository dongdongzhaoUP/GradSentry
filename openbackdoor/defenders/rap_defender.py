...
class RAPDefender(Defender):
    r"""
        ...
        RAP (Robustness-Aware Perturbations) detects backdoor samples by
        learning perturbations that cause different probability changes
        for clean vs poisoned samples.

        Codes adapted from RAP's official implementation.

    Args:
        epochs (`int`, optional): Number of RAP training epochs. Default to 5.
        batch_size (`int`, optional): Batch size. Default to 32.
        lr (`float`, optional): Learning rate for trigger embeddings. Default to 1e-2.
        triggers (`List[str]`, optional): Trigger words to insert. Default to ["cf"].
        target_label (`int`, optional): Target label for attack. Default to 1.
        prob_range (`List[float]`, optional): Probability change bounds. Default to [-0.1, -0.3].
        scale (`float`, optional): Scale factor for loss. Default to 1.
    ...
    def detect(
        ...
        """
        Detect poison samples using RAP.
        ...
        logger.info(f"Clean diff: {np.mean(clean_prob):.4f}, Poison diff: {np.mean(poison_prob):.4f}")
        logger.info(f"Clean ASR: {clean_asr:.4f}, Poison ASR: {poison_asr:.4f}")
    ...
    def construct(self, clean_dev):
        """Construct RAP perturbations by training trigger embeddings."""
        ...
        for epoch in range(self.epochs):
            ...
            logger.info(f"Epoch {epoch+1}: RAP loss = {epoch_loss:.4f}, success rate = {asr:.4f}")

    def rap_poison(self, data):
        """Insert trigger words at the beginning of each sentence."""
        rap_data = []
    ...
    def rap_prob(self, model, data, clean=True):
        """Calculate probability differences with RAP perturbations."""
        model.eval()
    ...
    def get_trigger_ind_norm(self, model):
        """Get trigger token indices and their embedding norms."""
        ind_norm = []
